    ...
    render() {
        ...
        return (
            <div className="profile">
                ...
                {this.props.state.me &&
                    this.props.state.friends_requests &&
                    this.props.state.friends_requests.map(email => {
                        return (
                            <div className=" user user-alert request" onClick={() => this.props.display(email)} key={findUser(email)._id}>
                                <div className="first">
                                ...
                                <button className="addFriend" onClick={() => this.props.acceptFriendRequest(email)} name="Accept friend " title="Accept friendship request " style={{ border: "2px solid " + this.props.state.me.color + "cc" }}>
                                    <FontAwesomeIcon icon={faUserCheck} />
                                </button>
                                <button className="addFriend" onClick={() => this.props.deniedFriendRequest(email)} name="Denied friendship request" title="Denied friendship request" style={{ border: "2px solid " + this.props.state.me.color + "cc" }}>
                ...
                {user && user.last_activity && Date.now() - user.last_activity < 120000 ? (
                    <div className="last-activity">
                        <span id="online-bullet">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/3/31/Button_Icon_Green.svg/768px-Button_Icon_Green.svg.png" alt="Online" title="Online" />
                        </span>
                        Online
                    </div>
                ) : (
                    <div className="last-activity">
                        Last activity: <br />
                        <b>{user && convertUNIX(user.last_activity)} </b>
                    </div>
                )}
                ...
                {user && user.tel ? <div className="tel">Phone: {user.tel}</div> : null}
                ...
                {this.props.state.display === this.props.state.me.email ? (
                    <div>
                        ...
                        <input type="color" style={{ width: " 30px", height: " 30px", borderRadius: "50%", display: "inline" }} name="favcolor" id="favcolor" defaultValue={this.props.state.color} onChange={this.props.updateData} />
                        <input
                            type="submit"
                            value="Save"
                            ...
                            .map((email, index) => {
                                ...
                                return (
                                    <div className="user" onClick={() => this.props.display(email)} key={index}>
                                        ...
                                        {this.props.state.me.requests_sent.some(el => el === email) || this.props.state.me.friends.some(elem => elem === email) ? (
                                            <button className="addFriend" title="Revoke friendship request" onClick={() => this.props.revokeFriendRequest(email)} style={{ border: "2px solid " + this.props.state.me.color + "cc" }}>
                                                <FontAwesomeIcon icon={faUserTimes} />
                                            </button>
                                        ) : (
                                            <button className="addFriend" title="Send friendship request" onClick={() => this.props.sendFriendRequest(email)} style={{ border: "2px solid " + this.props.state.me.color + "cc" }}>
                                                <FontAwesomeIcon icon={faUserCheck} />
                                            </button>
                                        )}
                                    </div>
